  ...
  return (
    <Card {...reveal(0, 'p-4')}>
      ...
      <div
        ...
        className="mt-3.5 h-1.5 overflow-hidden rounded-full bg-night-raise"
      >
        <div className="h-full rounded-full bg-signal" style={{ width: `${pct}%` }} />
      </div>

      <p className="mt-2.5 text-[11px] text-night-mute">
  ...
  return (
    <Card {...reveal(0, 'p-4')}>
      ...
      <p className="mt-2.5 flex items-baseline gap-1.5 leading-none">
        ...
        <span className="text-sm text-night-mute">dias</span>
      </p>
      ...
      <div className="mt-3.5 flex h-7 items-end gap-1" aria-hidden="true">
        {data.barras.map((value, i) => (
          <span
            key={i}
            className={`flex-1 rounded-t-[3px] ${i === data.destaque ? 'bg-white' : 'bg-signal'}`}
      ...
      {delta > 0 && (
        <p className={`mt-2.5 text-[11px] font-medium ${faster ? 'text-up' : 'text-down'}`}>
